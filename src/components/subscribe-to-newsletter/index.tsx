...
export const SubscribeToNewsletter = (props: SubscribeToNewsletterProps) => {
  const {title = 'KEEP CONNECTED', subTitle = 'Get updates by subscribe our weekly newsletter', placeholder = 'Your email address', btnText = 'Subscribe'} = props;
  ...
  const {register, handleSubmit, reset, formState: {errors}} = useForm<SubscriptionFormDataType>({
    defaultValues,
  })

  const {isLoading, mutate} = useMutation<AxiosResponse<{ msg: string }>, AxiosError<{ msg: string }>, SubscriptionFormDataType>(
    {
      mutationKey: 'submit',
      mutationFn: (data) => {
            ...
            <div className="newsletter_inner d-flex justify-content-between align-items-center">
              ...
              <div className="newsletter_subscribe">
                <form id="mc-form" onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <button className="border-0" id="mc-submit" disabled={isLoading}>{btnText}</button>
                </form>
